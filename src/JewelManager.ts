class JewelManager {
    private jewels: Jewel[][] | null[][] = new Array();
            ...
            for (let x = 0; x < this.numberOfJewelsHorizontal; x++) {
                for (let y = 0; y < this.numberOfJewelsVertical; y++) {
                    this.jewels[x][y] = new Jewel(
                        ...
                        `${this.graphicsPath}${Colors[Math.floor((Math.random() * Object.keys(Colors).length) / 2)]}-jewel-sprite-sheet.png`,
                        20,
                        false,
                        120,
                        true,
                    ...
                    if (this.checkConnectedJewels(x, y, this.jewels[x][y]!.color) >= 4) y--;
                    this.uncheckJewels();
                }
            }
